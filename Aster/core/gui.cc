...
void Init(Swapchain* _swapchain) {
	...
	ERROR_IF(failed(result), stl::fmt("Renderpass creation failed with %s", to_cstring(result))) THEN_CRASH(result) ELSE_INFO("UI pass Created");
	...
	result = cmd.end();
	ERROR_IF(failed(result), stl::fmt("Cmd buffer end failed with %s", to_cstring(result))) THEN_CRASH(result);

	auto task = SubmitTask<void>();
	result = task.submit(device_, device_->queues.transfer, device_->transfer_cmd_pool, { cmd });
	ERROR_IF(failed(result), stl::fmt("Fonts could not be loaded to GPU with %s", to_cstring(result))) THEN_CRASH(result);
	...
	for (auto& iv : _swapchain->image_views)
	{
		...
		ERROR_IF(failed(result), stl::fmt("GUI Framebuffer creation failed with %s", to_cstring(result))) THEN_CRASH(result);
	}

	result = task.wait();
	ERROR_IF(failed(result), stl::fmt("Fence wait failed with %s", to_cstring(result))) THEN_CRASH(result);
}
