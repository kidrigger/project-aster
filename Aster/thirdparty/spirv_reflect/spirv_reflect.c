    ...
    switch (p_node->op) {
      ...
      case SpvOpSource: {
        ...
        if (p_node->word_count >= 5) {
          ...
          size_t source_temp_len = strlen(p_source);
          char* p_source_temp = (char*)calloc(source_temp_len + 1, sizeof(char*));

          if (IsNull(p_source_temp)) {
            return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED;
          }

          strcpy_s(p_source_temp, source_temp_len * sizeof(char*), p_source);
      ...
      case SpvOpSourceContinued: {
        ...
        size_t continued_source_len = strlen(p_source) + strlen(p_parser->source_embedded);
        char* p_continued_source = (char*)calloc(continued_source_len + 1, sizeof(char*));

        if (IsNull(p_continued_source)) {
          return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED;
        }

        strcpy_s(p_continued_source, continued_source_len * sizeof(char*), p_parser->source_embedded);
        strcat_s(p_continued_source, continued_source_len, p_source);
        SafeFree(p_parser->source_embedded);
...
static SpvReflectResult ParseSource(Parser* p_parser, SpvReflectShaderModule* p_module)
{
  ...
  if (IsNotNull(p_parser) && IsNotNull(p_parser->spirv_code)) {
    ...
    //Source code
    size_t source_len = strlen(p_parser->source_embedded);
    ...
    strcpy_s(p_source, source_len * sizeof(char*), p_parser->source_embedded);
  ...
  switch (op_type) {
    ...
    case SpvOpTypeStruct: {
      ...
      if (index >= p_var->member_count) {
        // Note: Hack for https://github.com/KhronosGroup/SPIRV-Reflect/issues/78
        return SPV_REFLECT_RESULT_SUCCESS;
